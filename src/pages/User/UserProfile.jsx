...
const UserProfile = () => {
  ...
  const handleClick = async () => {
    try {
      const res = await customAxios.post("/profile/verify/request", {
        message: message,
      });
      console.log(res);
          ...
          <h3 className="mb-1.5 flex items-center justify-center gap-2 capitalize text-2xl font-semibold text-black dark:text-white">
            ...
            <div className="">
              {user?.is_verified == "pending" ? (
                ...
                <AlertDialog>
                  <AlertDialogTrigger className="flex items-center gap-1 rounded-full px-2 py-1 cursor-pointer border-dashed border-2 border-gray-500">
                    <NotVerifiedIcon />
                  ...
                  <AlertDialogContent>
                    <AlertDialogHeader>
                      <AlertDialogTitle>
                        add message to verify your account
                      </AlertDialogTitle>
                      <AlertDialogDescription>
                        <Textarea
                          value={message}
